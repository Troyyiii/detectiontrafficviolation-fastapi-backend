...
class DetectViolation:
    def __init__(self, travio_model):
        # Load custom trained YOLOv5 model
        self.travio_model = load_model(travio_model)
    ...
    def start_detect(self, frame):
        if not self.area:
            ...
            cvzone.putTextRect(processed_frame, f"TV: {self.traffic_light_violator_counter}, WWV: {self.wrong_way_violator_counter}, HV: {self.helmet_violator_counter}, NHc: {self.no_helmet_counter}", (20, 60), scale=0.8, thickness=1, offset=1)
